...
/**
 * Anti-CSRF token helpers for the **double-submit** pattern. `SameSite=Strict` on the session cookie
 * is the primary defense; this token is defense-in-depth required on every state-changing
 ...
 * `base64url`-encoded. The randomness comes from the injected {@link AuthCrypto} (a CSPRNG), never a
 * seeded RNG.
 */
