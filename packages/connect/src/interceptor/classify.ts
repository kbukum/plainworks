...
/**
 * Whether a failure is worth retrying, layered on the shared `std` classifier so connect uses **one**
 * retry taxonomy instead of forking its own. A `ConnectError` is judged by its {@link Code}; anything
 * else (a per-attempt `std` `TimeoutError` → retryable, a caller `AbortError` → fatal, an unknown
 * throw → fatal) defers to `std`'s `isRetryable`.
 */
