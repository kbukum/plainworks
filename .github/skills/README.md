...
| [`validate`](validate/SKILL.md) | Run the DoD gates (`check-versions · lint · check-comments · typecheck · check-boundaries · build · test · check-packaging`, plus `registry:validate` for atoms) through `bun run`/`turbo`, scoped to the changed package(s). |
...
| [`update-atoms`](update-atoms/SKILL.md) | Refresh the **vendored**, **locked** shadcn atoms with `registry:update` and reconcile on the deviation ladder — never by hand-editing an atom. |
