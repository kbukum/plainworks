...
- **Module cohesion.** A single file that has grown **over-long** and **mixes several distinct concerns** should be promoted to a folder (a re-export `index.ts` + concern-named modules). Length alone is never the verdict; concern-mixing is. Restructure in the same change that touches it rather than deferring. A cohesive single-concern file is fine at any length.
