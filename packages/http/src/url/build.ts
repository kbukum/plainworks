 ...
 * between {@link buildUrl} and the transport and can rewrite `request.url`, so the credential guards
 * are re-run on the final URL — userinfo credentials and credential-shaped query parameters are
 * rejected even when they were reintroduced downstream of the original build. A URL that no longer
 * parses is refused as fatal.
 */
...
/**
 * Reject a URL that embeds credentials in its userinfo (`https://user:pass@host`). Basic-auth in the
 * URL is a credential-in-URL just like a token in the query string — header-only auth forbids both.
 * The offending values are never echoed into the error message so they cannot leak through a log.
 */
 ...
 * raw parser error is carried onto the thrown error — a malformed URL can itself embed a password or
 * token, and the platform `TypeError` echoes the input verbatim in its message, so attaching it as
 * `cause` would leak that value through a log. The caller only learns that the supplied URL was
 * invalid.
 */
