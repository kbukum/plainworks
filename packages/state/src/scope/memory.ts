 ...
 * {@link createStore} — the same store model the React binding drives — so `memory` is genuinely one
 * more scope over the existing store, not a parallel state API bolted alongside it. The store starts
 * empty (`undefined`); `set` replaces the whole value (works for a primitive or an object), `remove`
 * clears it, and `subscribe` is the store's own subscription.
 */
 ...
 * value declared in this scope is the one that can hydrate straight from an SSR snapshot. Use it for
 * ephemeral UI state; reach for `persistent`/`session` when the value must outlive the page.
 */
