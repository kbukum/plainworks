...
describe("ThemeProvider", () => {
  ...
  it("resets to the initial when an external clear empties the source", async () => {
    installMatchMedia(false)
    const source = deferredStateSource<ThemePreference>({
      initial: { mode: "dark", colorScheme: "indigo" },
    })
    render(
      <ThemeProvider source={source} initialTheme={{ mode: "light", colorScheme: "indigo" }}>
        <ThemeConsumer />
      </ThemeProvider>,
    )
    await waitFor(() => expect(source.reads.length).toBe(1))
    source.reads[0]?.resolve({ mode: "dark", colorScheme: "indigo" })
    await waitFor(() => expect(document.documentElement.classList.contains("dark")).toBe(true))

    // An external writer clears the slot; the subscription-triggered read comes back empty.
    await source.set({ mode: "dark", colorScheme: "indigo" })
    await waitFor(() => expect(source.reads.length).toBe(2))
    source.reads[1]?.resolve(undefined)

    // A cleared slot must fall back to the configured initial, not strand the stale dark theme.
    await waitFor(() => expect(document.documentElement.classList.contains("dark")).toBe(false))
  })

  it("keeps a local write when a read already in flight resolves stale", async () => {
    installMatchMedia(false)
    // A source that does not echo the local writer: no subscription-triggered read masks the stale
    // mount read, so only local-write marking can protect the fresher value.
    const source = deferredStateSource<ThemePreference>({
      initial: { mode: "light", colorScheme: "indigo" },
      echoesLocalWrites: false,
    })
    render(
      <ThemeProvider source={source}>
        <ThemeConsumer />
    ...
    // The user switches to dark while the mount read is still parked on the old light value.
    await userEvent.setup().click(screen.getByRole("button", { name: "Use dark theme" }))
    await waitFor(() => expect(document.documentElement.classList.contains("dark")).toBe(true))

    // The stale mount read resolves last with the pre-write value; the local write must survive it.
    source.reads[0]?.resolve({ mode: "light", colorScheme: "indigo" })
    await waitFor(() => expect(source.reads[0]?.settled).toBe(true))
    expect(document.documentElement.classList.contains("dark")).toBe(true)
  })
