 ...
 * cache (so `dehydrateClient` will ship it); `failed` carries the rejection `cause` so the caller
 * can log or report it — the error is preserved, never swallowed into a bare boolean.
 */
...
/**
 * Prefetch a query into `client` on the server so its data is warm before render — the first half
 * of the RSC prefetch → dehydrate → hydrate flow. Built on TanStack's `client.query` (the
 * non-deprecated successor to `prefetchQuery`), which *rejects* on a fetch error; this wrapper
 * turns that rejection into a `failed` {@link PrefetchOutcome} carrying the cause, so a warming
 * failure never aborts the server render yet is never silently discarded. TanStack's default
 * dehydration serializes only **successful** queries, so a failed prefetch is not shipped to the
 * browser: the client mounts the query cold and fetches it fresh, where its own error boundary
 * handles a repeat failure. A server component can therefore `await` a batch of prefetches, inspect
 * the outcomes (e.g. to log failures), and {@link dehydrateClient} the lot without any one failure
 ...
 * trust boundary into the browser payload, so the query policy is **required**: there is no "ship
 * whatever succeeded" default to forget to tighten. Pass an allowlist
 * (`shouldDehydrateQuery: (query) => allowlist.has(query.queryKey[0])`) when any warmed query is
 * not public, or an explicit `() => true` when every query on this client is safe to ship.
 * Mutations are never dehydrated by this helper unless `shouldDehydrateMutation` is supplied
 * explicitly (TanStack's own default ships *paused* mutations, which a server render has no
 * business replaying).
 */
...
/**
 * Rehydrate a {@link DehydratedState} into `client` imperatively — the non-React path (a worker, a
 * test, or a host wiring hydration by hand). Inside a React tree, prefer the `HydrationBoundary`
 * component from `./client`, which calls this for you and re-runs it as streamed server data
 * arrives.
 */
