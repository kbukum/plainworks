...
- **Vendored atoms stay vendored.** `packages/elements/src/shadcn/` holds only locked shadcn CLI output; `src/atoms/` holds primitives we write. A diff under `src/shadcn/` (or to `shadcn.lock.json`) that `registry:update`/`registry:add` didn't produce — `registry:validate` reports a lock mismatch — is a **blocker**; so is an owned primitive placed in `src/shadcn/`, or one name in both folders. A deviation belongs on the **deviation ladder** (theme → call site → `ui` wrapper), never in the atom. See the [Vendored atoms](../../../copilot-instructions.md#vendored-atoms) baseline.
