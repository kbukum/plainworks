...
describe("bundleExample", () => {
  ...
  it("writes a clean standalone starter README", () => {
    bundleExample({ appDir, destDir, repoRoot })
    const readme = readFileSync(join(destDir, "README.md"), "utf8")
    expect(readme).toContain("# plainworks-app")
    expect(readme).toContain("bun run dev")
    expect(readme).not.toContain("--filter")
  })
