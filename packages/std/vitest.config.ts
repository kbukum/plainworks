...
export default defineConfig({
  test: {
    ...
    coverage: {
      ...
      // is proven by the CI dist check, not coverage); excluding every `index.ts`/`client.ts`
      // barrel — root or nested concern folder — keeps tests from coupling to a re-export file just
      // to color a line.
