// The injected WebSocket seam. `WebSocket` is NOT a universal primitive (absent on some server
// runtimes, and the browser constructor cannot attach request headers), so it is an injected
// factory with a platform default — the same "inject what varies by host" rule the SSE transport
// applies to `fetch`. Header-only auth flows through {@link WebSocketConnectInit.headers}: a Node
...
/**
 * The minimal `WebSocket` surface the transport drives: send text, close, read `readyState`, and
 * the four lifecycle handlers. A real platform `WebSocket` (browser or Node `ws`) satisfies it
 * structurally.
 */
export interface WebSocketLike {
  readonly readyState: number
  /**
   * Binary delivery mode; the transport sets `"arraybuffer"` so binary messages arrive decodable.
   * Left at the browser default (`"blob"`), a binary frame would surface as an undecodable `Blob`.
   */
  binaryType: string
...
/**
 * Constructs a {@link WebSocketLike}. Injected so a header-capable implementation (Node `ws`) can
 * be supplied where the browser constructor cannot attach headers, and so tests drive a fake
 * socket.
 */
...
/**
 * The platform default factory, built from `globalThis.WebSocket`. It cannot forward headers (the
 * browser constructor has no header argument), so a header-authenticated channel must inject a
 * header-capable factory instead.
 */
export function resolveGlobalSocketFactory(): WebSocketFactory {
  ...
  return (url, init) => {
    // The platform constructor has no header argument — fail rather than silently drop resolved
    // credentials and downgrade an authenticated attempt to anonymous.
