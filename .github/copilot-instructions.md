...
- **Host-independence (the axis):** every package ships a **server-safe `.` entry** (no React/DOM, RSC/edge-safe) and, where interactive, an optional **client `./client` entry** with per-module `"use client"`. Client is never the default import. Token-custody code (auth server) must never be pulled into a `"use client"` graph — this is an enforced import boundary, not a convention.
...
- **Files by concern.** A barrel `index.ts` re-exports; it holds no logic. Split an over-long, concern-mixed file into concern-named modules rather than piling into one.
