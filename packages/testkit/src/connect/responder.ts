 ...
 * exercise error mapping and retry classification without hand-writing a throwing closure.
 * Re-exports Connect's `Code` so a test names failures as `failUnary(Code.Unavailable)`.
 *
 ...
 * arbitrary async function, so the delay seam is injected there rather than through a bespoke
 * helper.
 */
