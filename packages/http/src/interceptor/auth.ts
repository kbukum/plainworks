 ...
 * header-only, never a token in the URL. The provider is consulted on every attempt (it sits below
 * the retry driver), so a refreshed credential is re-applied on a retry. The attempt's abort signal
 * is passed to the provider as its {@link AuthContext}, so an async token refresh is bound to the
 * same deadline/cancellation as the request — when the attempt times out or the caller aborts, an
 * abandoned refresh is cancelled instead of running on. A copy of the headers is used so the base
