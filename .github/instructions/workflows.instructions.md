...
- **Run the gates through the root scripts**, in the DoD order, so CI and local stay identical: `check-versions → lint → registry:validate (elements, verifies the shadcn lock) → check-comments → typecheck → check-boundaries → build → test`.
