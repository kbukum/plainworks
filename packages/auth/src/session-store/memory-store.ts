...
export interface MemorySessionStoreConfig {
  /**
   * The scope that holds the token; defaults to the neutral `memory` scope. Any scope passed here
   * is checked by the secret guard and rejected unless it is memory-equivalent, so the token can
   * never be routed to `localStorage`, a cookie, or the URL.
   */
 ...
 * `memory` scope and runs `@plainworks/state`'s **secret guard**
 * ({@link assertScopeAllowsSensitivity}) so the token can live *only* in a memory-equivalent scope
 * — no bespoke store, one enforcement path.
 *
 * Refresh custody stays server-side (a same-origin refresh endpoint); a pure no-backend SPA that
 * keeps tokens in JS storage is not a supported secure topology.
 *
 * @throws {import("@plainworks/state").StateConfigError} when a non-memory-equivalent `scope` is
 *   supplied — the guard refuses to place a secret anywhere a script, the server, or another tab
 *   can read it.
 */
