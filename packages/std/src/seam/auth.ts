 ...
 * provider that performs an async token refresh can bind the refresh to the same
 * deadline/cancellation as the request: when the attempt times out or the caller aborts, the
 * abandoned refresh is cancelled instead of running on after the request has moved on. A provider
 * that refreshes MUST honor `signal` (pass it to its own `fetch`/awaits, or check
 * `signal.throwIfAborted()`); a static provider may ignore it.
 */
 ...
 * request, or `undefined` when unauthenticated. It may be async (e.g. to refresh a token first); an
 * async refresh receives an {@link AuthContext} and must honor its `signal` so a refresh never
 * outlives the attempt that triggered it.
 */
