...
describe("targetName", () => {
  it("restores known dotfile markers to a dotfile", () => {
    expect(targetName("_gitignore")).toBe(".gitignore")
    expect(targetName("_npmrc")).toBe(".npmrc")
  })

  it("leaves legitimate underscore-prefixed files and ordinary names unchanged", () => {
    expect(targetName("_redirects")).toBe("_redirects")
    expect(targetName("_headers")).toBe("_headers")
