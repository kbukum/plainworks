...
import { isSkippedEntry, STANDALONE_TSCONFIG } from "./config"
...
export function bundleExample(options: BundleExampleOptions): void {
  ...
  // The base-extending workspace tsconfig is replaced with the inline standalone one.
  writeFileSync(
  ...
  writeFileSync(
    ...
    `${JSON.stringify(toTemplateManifest(appManifest), null, 2)}\n`,
  )
