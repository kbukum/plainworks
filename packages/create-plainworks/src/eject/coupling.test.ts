...
describe("assertEjectable", () => {
  ...
  it.each(["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"] as const)(
    ...
    (section) => {
      writeJson(join(appDir, "package.json"), {
        ...
        [section]: { "@plainworks/demo": "workspace:*" },
      })
  ...
  it("validates CSS @import and url references", () => {
    // Declared package CSS import is allowed.
    writeSource(
      join(appDir, "src", "globals.css"),
      '@import "@plainworks/ui/styles.css";\n@import url("https://fonts.googleapis.com/css");\n',
    )
    expect(() => assertEjectable({ appDir, repoRoot })).not.toThrow()

    // Undeclared package CSS import is rejected.
    writeSource(
      join(appDir, "src", "globals.css"),
      '@import "@plainworks/internal-pkg/styles.css";\n',
    )
    expect(() => assertEjectable({ appDir, repoRoot })).toThrow(/not a declared dependency/)

    // Workspace-escaping relative CSS import is rejected.
    writeSource(join(appDir, "src", "globals.css"), '@import "../../../packages/ui/styles.css";\n')
    expect(() => assertEjectable({ appDir, repoRoot })).toThrow(/resolves outside the app/)
  })
