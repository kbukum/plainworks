...
async function expectBody(url, expected) {
  const response = await fetch(url)
...
try {
  ...
  // Scaffold with the freshly built initializer, then point the pinned deps at the local tarballs
  // (transitive `@plainworks/*` deps inside the tarballs resolve through the same overrides).
  const cli = join(packageDir, "dist", "bin.js")
  run("node", [cli, "my-app", "--host", "next", "--no-install", "--no-git"], projects)
