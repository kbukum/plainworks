...
type ItemVariantSchema = {
  variant: Record<"default" | "outline" | "muted", string>
  size: Record<"default" | "sm" | "xs", string>
}

const itemVariants: ReturnType<typeof cva<ItemVariantSchema>> = cva<ItemVariantSchema>(
  "group/item flex w-full flex-wrap items-center rounded-lg border text-sm transition-colors duration-100 outline-hidden focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring [a]:transition-colors [a]:hover:bg-muted",
  {
    variants: {
      variant: {
...
type ItemMediaVariantSchema = {
  variant: Record<"default" | "icon" | "image", string>
}

const itemMediaVariants: ReturnType<typeof cva<ItemMediaVariantSchema>> =
  cva<ItemMediaVariantSchema>(
